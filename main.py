from fastapi import FastAPI
from fastapi.responses import HTMLResponse
from fastapi.staticfiles import StaticFiles
import uvicorn

app = FastAPI()
app.mount("/static", StaticFiles(directory="static"), name="static")

@app.get("/", response_class=HTMLResponse)
def read_root():
    return """
    <!doctype html>
    <html lang="en">
        <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <title>Bootstrap demo</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-4bw+/aepP/YC94hEpVNVgiZdgIC5+VKNBQNGCHeKRQN+PtmoHDEXuppvnDJzQIu9" crossorigin="anonymous">
            <link href="/static/style.css" rel="stylesheet">
            <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css">
        </head>
        <body class="custom-background">
            <nav class="navbar navbar-expand-lg bg-dark border-bottom border-body" data-bs-theme="dark">
              <div class="container-fluid">
                <a class="navbar-brand" href="#">MENU</a>
                <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                  <span class="navbar-toggler-icon"></span>
                </button>
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                  <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    <li class="nav-item">
                      <a class="nav-link active" aria-current="page" href="http://127.0.0.1:8000/">Home</a>
                    </li>
                    <li class="nav-item dropdown">
                      <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        Portfolio
                      </a>
                      <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="http://127.0.0.1:8000/website/">Website</a></li>
                        <li><a class="dropdown-item" href="http://127.0.0.1:8000/dataengineer/">Data_Engineer</a></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item" href="https://github.com/rlfahs3/USA-COVID19-">etc</a></li>
                      </ul>
                    </li>
                  </ul>
                  <form class="d-flex" role="search">
                    <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search">
                    <button class="btn btn-outline-success" type="submit">Search</button>
                  </form>
                </div>
              </div>
            </nav>
            <div class="contianer">
                <div class="jumbotron">
                    <h1 class="text-center white-text">RESUME</h1>
                    <p class="text-center white-text"><i class="fa-solid fa-database"></i>&nbsp;DATA ENGINEER</p>
                    <p class="text-center">
                        <a href="https://www.linkedin.com/in/chanhohyun/?originalSubdomain=kr" class="btn linkedin-button"><i class="fa-brands fa-linkedin"></i> LinkedIn</a>
                        <a href="https://www.instagram.com/hch16/?hl=ko" class="btn instagram-button"><i class="fa-brands fa-instagram"></i> Instagram</a>
                        <a href="https://www.facebook.com/chanho.hyun" class="btn facebook-button"><i class="fa-brands fa-facebook"></i> FaceBook</a></p>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div class="col-md-4">
                        <div class="card" style="width: 18rem;">
                            <img src="data:image/jpeg;base64," class="card-img-top" alt="...">
                            <div class="card-body">
                                <h5 class="card-title">Chanho Hyun</h5>
                                <p class="card-text"><i class="fa-regular fa-calendar-days"></i>&nbsp;&nbsp;1997-02-16
                                <p class="card-text"><i class="fa-solid fa-envelope"></i>&nbsp;&nbsp;my_name@gmail.com
                                <p class="card-text"><i class="fa-solid fa-phone"></i>&nbsp;&nbsp;010-1234-5678</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <div class="accordion custom-accordion" id="accordionFlushExample">
                          <div class="accordion-item">
                            <h2 class="accordion-header">
                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOne" aria-expanded="false" aria-controls="flush-collapseOne">
                                <i class="fa-solid fa-graduation-cap"></i>
                                &nbsp;&nbsp;Education
                              </button>
                            </h2>
                            <div id="flush-collapseOne" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                              <div class="accordion-body">University of Toronto st.george
                                <div class="row">
                                    <div class="col text-left">
                                        <p class="small transparent-text">
                                            Sep 2015 - Apr 2020
                                        </p>
                                    </div>
                                    <div class="col text-right">
                                        <p class="small transparent-text">
                                            <i class="fa-solid fa-user-graduate"></i>&nbsp;Bachelor of Science in Mathematics and Statistics
                                        </p>
                                    </div>
                                </div>
                                <ul>
                                    <li>Specialization: Mathematics, Data Analysis, Data Engineering</li>
                                </ul>
                              </div>
                            </div>
                          </div>
                          <div class="accordion-item">
                            <h2 class="accordion-header">
                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseTwo" aria-expanded="false" aria-controls="flush-collapseTwo">
                                <i class="fa-solid fa-pencil"></i>
                                &nbsp;&nbsp;Skills
                              </button>
                            </h2>
                            <div id="flush-collapseTwo" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                              <div class="accordion-body"><i class="fa-solid fa-language"></i>&nbsp;Languages:
                                <ul>
                                    <li>English</li>
                                     <li>Korean</li>
                                </ul>
                              </div>
                              <div class="accordion-body"><i class="fa-solid fa-laptop-code"></i>&nbsp;Programming:
                                <ul>
                                    <li>R Studio</li>
                                    <li>Python</li>
                                    <li>Shell-Script</li>
                                    <li>Airflow</li>
                                    <li>Elasticsearch</li> 
                                    <li>Fastapi</li> 
                                    <li>Bootstrap</li>
                              </div>
                              <div class="accordion-body"><i class="fa-brands fa-microsoft"></i>&nbsp;OS and Microsoft office:
                                <ul>
                                    <li>linux</li>
                                    <li>windows</li>
                                    <li>macos</li>
                                    <li>Word</li>
                                    <li>PowerPoint</li>
                                    <li>Excel</li>
                                </ul>
                              </div>
                            </div>
                          </div>
                          <div class="accordion-item">
                            <h2 class="accordion-header">
                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseThree" aria-expanded="false" aria-controls="flush-collapseThree">
                                <i class="fa-solid fa-briefcase"></i>
                                &nbsp;&nbsp;Employment History
                              </button>
                            </h2>
                            <div id="flush-collapseThree" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                              <div class="accordion-body">Junior Data Engineer, NHN_DATA
                                <p class="small transparent-text">June 2022 - now</p>
                                    <ul>
                                        <li>Worked as a productive and positive team member to test, report and debug operations.</li>
                                        <li>Planned, developed, tested, and maintained the pipe-lines of ETL process for 'Payco', and 'Hangame' services.</li>
                                        <li>Helped achieve a consistent and stable system across the ETL process by operating a large-scale refactoring of the code.</li>
                                    </ul>        
                              </div>
                            </div>
                          </div>
                          <div class="accordion-item">
                            <h2 class="accordion-header">
                              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseFour" aria-expanded="false" aria-controls="flush-collapseFour">
                                <i class="fa-solid fa-gear"></i>
                                &nbsp;&nbsp;Extracurricular Activity
                              </button>
                            </h2>
                            <div id="flush-collapseFour" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                              <div class="accordion-body">UTKSA (University of Toronto Korean Student Association)
                                <div class="row">
                                    <div class="col text-left">
                                        <p class="small transparent-text">
                                            Sep 2015 - Apr 2018
                                        </p>
                                    </div>
                                    <div class="col text-right">
                                        <p class="small transparent-text">
                                            <i class="fa-solid fa-user-gear"></i>&nbsp;chief sciecne director
                                        </p>
                                    </div>
                                </div>
                                    <ul>
                                        <li> Offered high quality tutorial of MAT135,136 (AP Calculus A,B : first year mathematics cources in University of Toronto) for two years with self-made guide book of calculus</li>
                                        <li> Held a long-term Science Mentorship Seminar Program for University of Toronto students. It was mentor-mentee interactions with mentors who are University of Toronto alumni working in financial, dental, and medical fields.</li>
                                        <li> Managed and hosted an Information Seminar of the University of Toronto for high school students interested in university admission</li>
                                    </ul>        
                              </div>
                            </div>
                          </div>
                        </div>
                    </div>
                </div>
            </div>
            <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/js/bootstrap.bundle.min.js" integrity="sha384-HwwvtgBNo3bZJJLYd8oVXjrBZt8cqVSpeBNS5n7C8IVInixGAoxmnlMuBnhbgrkm" crossorigin="anonymous"></script>
        </body>
    </html>
    """

@app.get("/website", response_class=HTMLResponse)
def read_root():
    return """
    <!doctype html>
    <html lang="en">
        <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <title>Bootstrap demo</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-4bw+/aepP/YC94hEpVNVgiZdgIC5+VKNBQNGCHeKRQN+PtmoHDEXuppvnDJzQIu9" crossorigin="anonymous">
        </head>
        <body>
            <h>hi, welcome to my playground</h>
            <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/js/bootstrap.bundle.min.js" integrity="sha384-HwwvtgBNo3bZJJLYd8oVXjrBZt8cqVSpeBNS5n7C8IVInixGAoxmnlMuBnhbgrkm" crossorigin="anonymous"></script>
        </body>
    </html>
    """

@app.get("/dataengineer", response_class=HTMLResponse)
def read_root():
    return """
    <!doctype html>
    <html lang="en">
        <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <title>Bootstrap demo</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-4bw+/aepP/YC94hEpVNVgiZdgIC5+VKNBQNGCHeKRQN+PtmoHDEXuppvnDJzQIu9" crossorigin="anonymous">
        </head>
        <body>
            <h1>Coming Soon!</h1>
            <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/js/bootstrap.bundle.min.js" integrity="sha384-HwwvtgBNo3bZJJLYd8oVXjrBZt8cqVSpeBNS5n7C8IVInixGAoxmnlMuBnhbgrkm" crossorigin="anonymous"></script>
        </body>
    </html>
    """

if __name__ == "__main__":
    uvicorn.run("main:app",host="127.0.0.1",port=8000,reload=True)